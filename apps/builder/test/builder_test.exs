defmodule Helix.Test.BuilderTest do
  ...
  test "create class with is_system uses sys db prefix" do
    {:ok, %{sql_table: table_name}} = Builder.create_class(%Class{name: "ab", is_system: true, properties: []})
  ...
  test "create class with not is_system uses public db prefix" do
    {:ok, %{sql_table: table_name}} = Builder.create_class(%Class{name: "ab", is_system: false, properties: []})
  ...
  test "creating a class inserts object mapping for class" do
    {:ok, %{new_class: test_class}} = Builder.create_class(%Class{name: "test class", properties: []})
    mapping = SqlHelpers.class_mapping(test_class.id)
    assert mapping == {"public", "test_class", nil}
  end

  test "create column in system table" do
    {:ok, %{new_class: test_class}} = Builder.create_class(%Class
      {
        name: "test",
        is_system: true,
        properties: [
  ...
  test "create similar property in class fails" do
    ...
    {:error, :duplicated_property, _error} = Builder.create_properties(%ClassIdentifier{id: test_class.id}, props)
    columns = SqlHelpers.table_columns("sys", "test")
    assert columns == [
      {"id", "integer", nil, 32, 0},
  ...
  test "creating a class inserts object mapping for properties" do
    ...
    {:ok, _} = Builder.create_class(%Class{name: "test class", is_system: true, properties: [
    ...
    assert multiple_select_mapping == [{"multiple select", "sys", "test_class_multiple_select", nil}]
  end
